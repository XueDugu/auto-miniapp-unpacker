...
def get_file(event=None):
    flag=0
    file = file_entry.get()
    if(file=="请输入文件夹完整路径") or not os.path.isdir(file):
...
def start_listen():
    ...
    for _ in range(raw):    
        ...
        # text_box.insert(tk.END, "{package}\n".format(package=pkg))
    t=time.time()
    while listening:
        result=wechat_num()
        if result > raw:
            ...
            for i in range(len(pkg_list)):
                ...
                file = file_entry.get()
                # command = 'adb pull /data/data/com.tencent.mm/MicroMsg/{user}/appbrand/pkg/ {dest}'.format(pkg=pkg_list[i], dest=file)
...
def Auto():
    ...
    for f in matching_files:
        ...
        if unpacker_path=="输入解包工具地址" or not os.path.isdir(file):
